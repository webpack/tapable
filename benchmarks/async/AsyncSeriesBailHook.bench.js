...
async function main() {
	...
	bench.add("AsyncSeriesBailHook#callAsync (10 sync taps, no bail)", () =>
		callAsyncPromisified(noBail, 1)
	);

	const bailMid = makeHook(10, "sync", 4);
	bench.add("AsyncSeriesBailHook#callAsync (10 sync taps, bail mid)", () =>
		callAsyncPromisified(bailMid, 1)
	);

	const asyncNoBail = makeHook(5, "async", -1);
	bench.add("AsyncSeriesBailHook#callAsync (5 async taps, no bail)", () =>
		callAsyncPromisified(asyncNoBail, 1)
	);

	const asyncBail = makeHook(5, "async", 2);
	bench.add("AsyncSeriesBailHook#callAsync (5 async taps, bail mid)", () =>
