...
class HookCodeFactory {
	...
	setup(instance, options) {
		const { taps } = options;
		const { length } = taps;
		// `new Array(length)` preallocates the backing store directly.
		// `Array.from({length})` would fill each slot with `undefined`
		// through the array-like protocol before the loop overwrites them.
	...
	contentWithInterceptors(options) {
		if (this.options.interceptors.length > 0) {
			...
			// Mutate `options` directly instead of `Object.assign(options, {…})`
			// so we don't allocate the RHS literal.
			options.onError =
				onError &&
				((err) => {
					let code = "";
