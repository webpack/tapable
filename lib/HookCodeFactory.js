...
/**
 * @typedef {object} TapsSeriesOptions
 * @property {((tapIndex: number, error: string, next: () => string, doneBreak: (skipDone: boolean) => string) => string)=} onError generates per-tap error code
 * @property {((tapIndex: number, result: string, next: () => string, doneBreak: (skipDone: boolean) => string) => string)=} onResult generates per-tap result code
 * @property {boolean=} resultReturns true if the generated function returns a result
 * @property {OnDoneCallback=} onDone generates code for completion
 * @property {boolean=} doneReturns true if the generated function returns when done
 * @property {boolean=} rethrowIfPossible true to rethrow errors when safe
...
class HookCodeFactory {
	/**
	 * @param {EXPECTED_OBJECT=} config optional configuration object passed through by subclasses
	 */
	constructor(config) {
	...
	setup(instance, options) {
		...
		instance._x = /** @type {EXPECTED_FUNCTION[]} */ (fns);
	}
	...
	callTap(tapIndex, { onError, onResult, onDone, rethrowIfPossible }) {
		...
		const tap = /** @type {FullTap & { context?: boolean }} */ (
			opts.taps[tapIndex]
		);
		switch (tap.type) {
			case "sync":
				...
				if (!rethrowIfPossible) {
					...
					code += /** @type {OnErrorCallback} */ (onError)("_err");
					code += "}\n";
			...
			case "async": {
				...
				cbCode += /** @type {OnErrorCallback} */ (onError)(`_err${tapIndex}`);
				cbCode += "} else {\n";
				if (onResult) {
			...
			case "promise":
				...
				code += /** @type {OnErrorCallback} */ (onError)(
	...
	/**
	 * @param {TapsSeriesOptions} options series-call options
	...
	callTapsSeries({
		...
		if (tapsLength === 0) return /** @type {OnDoneCallback} */ (onDone)();
		...
		const doneBreak = (skipDone) => {
			if (skipDone) return "";
			return /** @type {OnDoneCallback} */ (onDone)();
		};
		let code = "";
		let current = onDone;
		let unrollCounter = 0;
		for (let j = tapsLength - 1; j >= 0; j--) {
			...
			if (unroll) {
				...
				code += /** @type {OnDoneCallback} */ (current)();
				code += "}\n";
				current = () => `${somethingReturns ? "return " : ""}_next${i}();\n`;
			}
			const done = /** @type {OnDoneCallback} */ (current);
			const content = this.callTap(i, {
				onError: (error) =>
					/** @type {NonNullable<TapsSeriesOptions["onError"]>} */ (onError)(
						i,
						error,
						done,
						doneBreak
	...
	/**
	 * @param {TapsSeriesOptions} options looping-call options
	 * @returns {string} generated code that calls taps in a loop
	 */
	callTapsLooping({ onError, onDone, rethrowIfPossible }) {
		const opts = /** @type {CompileOptions} */ (this.options);
		if (opts.taps.length === 0) return /** @type {OnDoneCallback} */ (onDone)();
		const syncOnly = opts.taps.every((t) => t.type === "sync");
		let code = "";
		if (!syncOnly) {
	...
	/**
	 * @param {TapsSeriesOptions & { onTap?: (tapIndex: number, run: () => string, done: () => string, doneBreak: (skipDone: boolean) => string) => string }} options parallel-call options
				...
				() =>
					this.callTap(i, {
						onError: (error) => {
							let code = "";
							code += "if(_counter > 0) {\n";
							code += /** @type {NonNullable<TapsSeriesOptions["onError"]>} */ (
								onError
							)(i, error, done, doneBreak);
							code += "}\n";
							return code;
						},
						onResult:
