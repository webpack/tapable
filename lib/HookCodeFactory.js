...
class HookCodeFactory {
	...
	contentWithInterceptors(options) {
		if (/** @type {CompileOptions} */ (this.options).interceptors.length > 0) {
			...
			// `content` is intentionally not declared on the base class - it's
			// abstract and provided by every subclass. Skipping the runtime
			// stub keeps the prototype chain mononomorphic on the hot compile
			// path: subclass `content` is the only one V8 sees, no shadowing.
			// @ts-expect-error abstract method, provided by subclasses for performance reasons
			code += this.content(
