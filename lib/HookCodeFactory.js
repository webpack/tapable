...
class HookCodeFactory {
	...
	contentWithInterceptors(options) {
		...
		// Fast path: no interceptors, skip the whole closure-wrapping dance.
		if (interceptorsLength === 0) return this.content(options);
		const { onError, onResult, onDone } = options;
		let code = "";
		for (let i = 0; i < interceptorsLength; i++) {
	...
	callTapsLooping({ onError, onDone, rethrowIfPossible }) {
		...
		const tapsLength = taps.length;
		if (tapsLength === 0) return onDone();
		// Inlined Array#every to avoid the callback allocation.
		let syncOnly = true;
		for (let i = 0; i < tapsLength; i++) {
			if (taps[i].type !== "sync") {
				syncOnly = false;
