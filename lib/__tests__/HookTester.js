...
class HookTester {
	...
	async runAsync(result, type) {
...

		{
			const hook = this.createHook(
				["x"],
				`${type}SinglePromiseWithUndefinedArg`
			);
			hook.tapPromise("promise", (arg) => {
				result[`${type}SinglePromiseWithUndefinedCalled1`] = arg;
				return Promise.resolve(undefined);
			});
			result[`${type}SinglePromiseWithUndefined`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}
...

		{
			const hook = this.createHook(
				["x"],
				`${type}SinglePromiseWithEmptyStringArg`
			);
			hook.tapPromise("promise", (arg) => {
				result[`${type}SinglePromiseWithEmptyStringArgCalled1`] = arg;
				return Promise.resolve("");
			});
			result[`${type}SinglePromiseWithEmptyStringArg`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}
