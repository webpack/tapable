...
class HookTester {
	...
	async runSync(result, method) {
...

		{
			...
			hook.intercept({
				...
				tap: (tap) => {
					result[`${method}InterceptedTap1`] = { ...tap, fn: tap.fn.length };
				}
			});
			hook.intercept({
				call: (a, b, c) => {
				...
				tap: (tap) => {
					if (!result[`${method}InterceptedTap2`]) {
						result[`${method}InterceptedTap2`] = { ...tap, fn: tap.fn.length };
					}
				}
			});
	...
	async runAsync(result, type) {
...

		{
			...
			result[`${type}MultipleSyncWithArgNoReturn`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}
...

		{
			...
			hook.tapPromise("promise", (_arg) => {
				return Promise.reject(null);
			});
			result[`${type}SinglePromiseWithNullReject`] = await this.gainResult((cb) =>
				hook[type](null, cb)
			);
		}

		{
			const hook = this.createHook(["x"], `${type}SinglePromiseWithUndefinedReject`);
			hook.tapPromise("promise", (_arg) => {
				return Promise.reject(undefined);
			});
			result[`${type}SinglePromiseWithUndefinedReject`] = await this.gainResult((cb) =>
				hook[type](null, cb)
			);
		}

		{
			...
			result[`${type}SinglePromiseWithZeroReject`] = await this.gainResult((cb) =>
				hook[type](null, cb)
			);
		}

		{
			const hook = this.createHook(["x"], `${type}SinglePromiseWithFalseReject`);
			hook.tapPromise("promise", (_arg) => {
				return Promise.reject(false);
			});
			result[`${type}SinglePromiseWithFalseReject`] = await this.gainResult((cb) =>
				hook[type](null, cb)
			);
		}

		{
			const hook = this.createHook(["x"], `${type}SinglePromiseWithEmptyReject`);
			hook.tapPromise("promise", (_arg) => {
				return Promise.reject("");
			});
			result[`${type}SinglePromiseWithEmptyReject`] = await this.gainResult((cb) =>
				hook[type](null, cb)
			);
		}
...

		{
			...
			result[`${type}MultipleMixedError1WithArg`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}

		{
			...
			result[`${type}MultipleMixedError2WithArg`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}

		{
			...
			result[`${type}MultipleMixedError3WithArg`] = await this.gainResult(
				(cb) => hook[type](42, cb)
			);
		}
	...
	async runIntercept(result, type) {
		{
			...
			hook.intercept({
				...
				tap: (tap) => {
					result[`${type}InterceptedTap1`] = { ...tap, fn: tap.fn.length };
				}
			});
			hook.intercept({
				call: (a, b, c) => {
				...
				tap: (tap) => {
					if (!result[`${type}InterceptedTap2`]) {
						result[`${type}InterceptedTap2`] = { ...tap, fn: tap.fn.length };
					}
				}
			});
