...
describe("Hook", () => {
	...
	it("should call tapAsync and tapPromise through withOptions wrapper", () => {
		const hook = new SyncHook();
		const tapAsync = jest.spyOn(hook, "tapAsync").mockImplementation(() => {});
		const tapPromise = jest
			.spyOn(hook, "tapPromise")
			.mockImplementation(() => {});

		const wrapped = hook.withOptions({ stage: 1 });
		const fn = () => {};
		wrapped.tapAsync("A", fn);
		wrapped.tapPromise("B", fn);

		expect(tapAsync).toHaveBeenCalledWith({ stage: 1, name: "A" }, fn);
		expect(tapPromise).toHaveBeenCalledWith({ stage: 1, name: "B" }, fn);
	});
