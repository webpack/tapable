...
class Hook {
	...
	_insert(item) {
		...
		// When `before` is a single string we hold it in a local string so
		// the walk can do `tap.name === beforeName` instead of allocating
		// a Set just to call `.has()` once per step. Multi-name `before`
		// still uses a Set.
		let beforeName;
		let before;
		if (typeof item.before === "string") {
			beforeName = item.before;
		...
		// Fork the walk by whether there's a `before` constraint. The
		// stage-only branch (no before, hit by `tap({stage})` insertions)
		// must not pay per-iteration overhead for the unused before-check,
		// otherwise it regresses vs the pre-fast-path loop.
		if (beforeName === undefined && before === undefined) {
			while (i > 0) {
				i--;
				const tap = taps[i];
				taps[i + 1] = tap;
				if ((tap.stage || 0) <= stage) {
					i++;
					break;
				}
			}
			...
			while (i > 0) {
				...
				if (beforeName !== undefined) {
					if (tap.name === beforeName) beforeName = undefined;
					continue;
				}
				if (before !== undefined) {
