...
class Hook {
	...
	_tap(type, options, fn) {
		if (typeof options === "string") {
			...
			if (
				...
				options.name = name;
				// Preserve previous precedence: user-provided keys win over the
				// internal `type`/`fn`.
	...
	_insert(item) {
		...
		const hasBefore =
			typeof itemBefore === "string" || Array.isArray(itemBefore);
		const itemStage = typeof item.stage === "number" ? item.stage : 0;
		...
		if (!hasBefore) {
			const n = taps.length;
			if (n === 0 || (taps[n - 1].stage || 0) <= itemStage) {
				taps[n] = item;
				return;
			}
		}
		...
		let before;
		if (typeof itemBefore === "string") {
			before = new Set([itemBefore]);
		} else if (Array.isArray(itemBefore)) {
			before = new Set(itemBefore);
		}
		let i = taps.length;
		while (i > 0) {
