...
class Hook {
	...
	_insert(item) {
		...
		// Single-name fast path (both `before: "x"` and `before: ["x"]`)
		// avoids allocating a Set - the loop just does a string compare.
		let beforeName;
		let before;
		if (typeof item.before === "string") {
			beforeName = item.before;
		} else if (Array.isArray(item.before)) {
			if (item.before.length === 1) {
				[beforeName] = item.before;
			} else {
