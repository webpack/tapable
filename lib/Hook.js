 ...
 * @property {((tap: FullTap & AdditionalOptions) => void)=} tap called for each tap before it runs
 ...
 * @property {((tap: FullTap & AdditionalOptions) => FullTap & AdditionalOptions)=} register called whenever a tap is registered
 ...
 * @property {HookInterceptor<EXPECTED_ANY, EXPECTED_ANY, EXPECTED_ANY>[]} interceptors the registered interceptors
 * @property {string[]} args names of the hook arguments (used for code generation)
 ...
 * @param {...EXPECTED_ANY} args hook call arguments
 * @returns {EXPECTED_ANY} hook call result
 */
 ...
 * @this {Hook<EXPECTED_ANY, EXPECTED_ANY, EXPECTED_ANY>}
 * @param {...EXPECTED_ANY} args hook call arguments
 * @returns {Promise<EXPECTED_ANY>} resolves with the hook result
 */
...
class Hook {
	/**
	 * @param {string[]=} args argument names of the hook (used for code generation)
	...
	/**
	 * @abstract
	 * @param {CompileOptions} _options compile options
	 ...
	 * @param {string | (Tap & AdditionalOptions)} options name or full tap options
	...
	_tap(type, options, fn) {
		if (typeof options === "string") {
			...
			options = /** @type {Tap & AdditionalOptions} */ ({ type, fn, name });
		} else {
		...
		options = this._runRegisterInterceptors(
			/** @type {FullTap & AdditionalOptions} */ (options)
		);
		this._insert(options);
	}

	/**
	 * @param {string | (Tap & AdditionalOptions)} options tap name or full tap options
	...
	/**
	 * @param {FullTap & AdditionalOptions} options the tap descriptor
	 * @returns {FullTap & AdditionalOptions} possibly transformed options
	 */
	_runRegisterInterceptors(options) {
		...
		for (let i = 0; i < length; i++) {
			...
			if (interceptor.register) {
				const newOptions =
					/** @type {FullTap & AdditionalOptions | undefined} */ (
	...
	/**
	 * @param {TapOptions & AdditionalOptions} options the options to merge into each tap
	...
	/**
	 * @param {FullTap & AdditionalOptions} item the tap to insert into the ordered taps list
	 * @returns {void}
	 */
	_insert(item) {
		this._resetCompilation();
