...
function CALL_DELEGATE(...args) {
	this.call = /** @type {CallDelegate<T, R>} */ (this._createCall("sync"));
...
function CALL_ASYNC_DELEGATE(...args) {
	this.callAsync = /** @type {CallAsyncDelegate<T, R>} */ (
		this._createCall("async")
	);
...
function PROMISE_DELEGATE(...args) {
	this.promise = /** @type {PromiseDeledate<T, R>} */ (
		this._createCall("promise")
	);
...
class Hook {
	 ...
	 */
	constructor(
		args = /** @type {ArgumentNames<AsArray<T>>} */ (
			/** @type {unknown} */ ([])
		),
		name = undefined
	) {
	...
	withOptions(options) {
		/**
		 * @param {string | (TapOptions & IfSet<AdditionalOptions>)} opt options
		 * @returns {Tap & IfSet<AdditionalOptions>} merged options
		 */
		const mergeOptions = (opt) =>
			/** @type {Tap & IfSet<AdditionalOptions>} */ (
				Object.assign(
					{},
					options,
	...
	intercept(interceptor) {
		...
		if (interceptor.register) {
			for (let i = 0; i < this.taps.length; i++) {
				this.taps[i] = interceptor.register(
					/** @type {FullTap & IfSet<AdditionalOptions>} */ (this.taps[i])
				);
			}
		}
	}
